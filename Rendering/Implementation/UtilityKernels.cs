...
namespace NullEngine.Rendering.Implementation
{
    ...
    {
        public static void ClearByteFramebuffer(LongIndex1 index, dByteFrameBuffer frameBuffer, byte r, byte g, byte b)
        {
            //FLIP Y
            //int x = (frameBuffer.width - 1) - ((index) % frameBuffer.width);
            int y = (frameBuffer.height - 1) - (((int)index) / frameBuffer.width);

            //NORMAL X
            int x = (((int)index) % frameBuffer.width);
